...
class LLMCache:
    ...
    def save(self) -> None:
        """Save cache to disk, merging with existing data to reduce clobbering in multiprocessing."""
        ...
        try:
            # Merge with current disk content to avoid losing other processes' updates
            if os.path.exists(self.cache_file):
                try:
                    with open(self.cache_file, "r") as f:
                        on_disk = json.load(f)
                        if isinstance(on_disk, dict):
                            # Update in-memory cache with anything new from disk
                            # but keep current in-memory as priority for what we just found
                            new_merged = on_disk
                            new_merged.update(self.cache)
                            self.cache = new_merged
                except (json.JSONDecodeError, IOError) as e:
                    logger.debug("Could not load existing cache for merge: %s", e)
