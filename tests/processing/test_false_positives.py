...
def test_keyword_filtering_skips_marketing(email):
    ...
    assert extractor._should_skip_llm_analysis(email_content) is True, f"Keyword filtering should have skipped {email['name']}"
...
def test_is_crypto_purchase_email_returns_false(email):
    ...
    mock_llm = MagicMock()
    mock_llm.generate_json.return_value.data = {"is_crypto_purchase": False, "confidence": 0.1, "reasoning": "Marketing"}
    ...
    assert mock_llm.generate_json.call_count == 0, f"LLM should not have been called for {email['name']} due to keyword filtering"
