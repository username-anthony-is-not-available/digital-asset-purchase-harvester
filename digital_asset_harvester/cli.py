...
def _init_worker(settings: HarvesterSettings) -> None:
    ...
    # Disable cache auto-save in workers to avoid race conditions and redundant I/O
    llm_client = get_llm_client(settings=settings, enable_cache_auto_save=False)
...
def process_emails(
    ...
    # Ensure LLM cache is persisted
    if settings.enable_llm_cache:
        from digital_asset_harvester.llm.cache import LLMCache
            ...
            if report.get("success"):
                ...
                for asset, data in report.get("results", {}).items():
                    ...
                    if status == "match":
                        ...
                        logger.warning(
                            f"  {asset}: DISCREPANCY! (Harvested: {harvested}, On-chain: {on_chain}, Diff: {diff})"
                        )
