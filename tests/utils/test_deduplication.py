...
class TestDeduplication(unittest.TestCase):
    ...
    def test_generate_record_hash(self):
        ...
        # Verify all composite fields contribute to uniqueness
        base_record = {
            "vendor": "V",
            "item_name": "I",
            "amount": "1.0",
        ...
        base_hash = generate_record_hash(base_record)

        for field in base_record:
            alt_record = base_record.copy()
            alt_record[field] = "DIFFERENT"
            self.assertNotEqual(base_hash, generate_record_hash(alt_record), f"Hash should change when {field} changes")
